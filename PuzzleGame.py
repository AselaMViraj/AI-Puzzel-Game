...
class Tiles(Label):
    ...
    def getTileAroundGap(self):
        ...
        return self.getTile((gRow, gCol - 1)), self.getTile((gRow - 1, gCol)), self.getTile(
            (gRow, gCol + 1)), self.getTile((gRow + 1, gCol))
...
class Board(Frame):
    ...
    def __init__(self, parent, image, grid, win, *args, **kwargs):
        ...
        self.grid = grid
        self.win = win
        Button(self, text="GOAL STATE", command=lambda: self.openCSV(), font=("Times New Roman", 14, "bold"),
               bg='azure3', width=13).grid(row=5, column=0)
        Button(self, text="SOLVE", command=lambda: self.solveBest(), font=("Times New Roman", 14, "bold"), bg='azure3',
               width=13).grid(row=5, column=2)

        self.label = Label(self, text="0 MOVES", font=("Times New Roman", 20, "bold"), fg='gray85', bg="gray85",
                           width=9)
    ...
    def openCSV(self):
        ...
        file_location = script_location / 'State.csv'
            ...
            if not openNodes.empty():
                ...
                if chosenNode == goalNode:
                    ...
                    for j in range(0, len(validMoves)):
                        ...
                        temp[i], temp[validMoves[j]] = temp[validMoves[j]], temp[i]  # swap
...
class Main():
    ...
    def createWidgets(self):
        self.mainFrame = Frame(self.parent, bg='aliceblue')
        Label(self.mainFrame, text='AI\n8 PUZZLE', font=("Times New Roman", 30, "bold"), fg='black',
              bg='aliceblue').pack(padx=10, pady=10)
        Label(self.mainFrame, text='by', font=("Times New Roman", 12, "bold"), bg='aliceblue', fg='black').pack(padx=10,
                                                                                                                pady=10)
        Label(self.mainFrame, text='DEEGHAYU B.K.\t\t\t(2017/E/024)', font=("Times New Roman", 14, "bold"),
              bg='aliceblue', fg='black').pack(padx=10, pady=10)
        Label(self.mainFrame, text='KUMARA K.J.A.M.V.\t\t(2018/E/067)', font=("Times New Roman", 14, "bold"),
              bg='aliceblue', fg='black').pack(padx=10, pady=10)
        Label(self.mainFrame, text='KUMARI T.B.S\t\t\t(2018/E/069)', font=("Times New Roman", 14, "bold"),
              bg='aliceblue', fg='black').pack(padx=10, pady=10)
        Label(self.mainFrame, text='RUWANPATHIRANA R.P.N.P\t(2018/E/103)', font=("Times New Roman", 14, "bold"),
              bg='aliceblue', fg='black').pack(padx=10, pady=10)
        Label(self.mainFrame, text='WICKRAMASINGHE W.M.R.S.N.\t(2018/E/141)', font=("Times New Roman", 14, "bold"),
              bg='aliceblue', fg='black').pack(padx=10, pady=10)
        frame = Frame(self.mainFrame, bg='gray85')
        frame.pack(padx=40, pady=40)

        Button(self.mainFrame, text="BEGIN", command=self.start, font=("Times New Roman", 40, "bold"),
               bg='azure3').pack(padx=30, pady=30)
        self.mainFrame.pack()
        self.board = Frame(self.parent, bg='aliceblue')
        self.winFrame = Frame(self.parent, bg='aliceblue')
        Label(self.winFrame, textvariable=self.winText, font=('', 35, "bold"), bg='aliceblue', fg='black').pack(padx=30,
                                                                                                                pady=30)
        Button(self.winFrame, text="PLAY AGAIN", command=self.home, font=("Times New Roman", 20, "bold"), bg='azure3',
               fg='black').pack(padx=10, pady=10)
        Button(self.winFrame, text="QUIT", command=root.destroy, font=("Times New Roman", 20, "bold"), bg='azure3',
    ...
    def win(self, moves):
        ...
        self.winText.set("Successfully finished\n{0} moves".format(moves))
        self.winFrame.pack()

    # home button's method
    def home(self):
